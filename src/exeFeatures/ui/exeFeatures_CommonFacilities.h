...
#ifndef exeFeatures_CommonFacilities_h
#define exeFeatures_CommonFacilities_h
...
DEFINE_STANDARD_HANDLE(exeFeatures_CommonFacilities, Standard_Transient)

//! Common tools and objects for A-Situs application.
class exeFeatures_CommonFacilities : public Standard_Transient
{
public:

  // OCCT RTTI
  DEFINE_STANDARD_RTTI_INLINE(exeFeatures_CommonFacilities, Standard_Transient)
...
public:
  ...
  //---------------------------------------------------------------------------
    ...
    void ActualizeAll()
    {
      if ( Part )   Part   ->Actualize(exeFeatures_CommonFacilities::Instance()->Model->GetRootNode(), true);
      if ( Domain ) Domain ->Actualize(exeFeatures_CommonFacilities::Instance()->Model->GetRootNode(), true);
      if ( Host )   Host   ->Actualize(exeFeatures_CommonFacilities::Instance()->Model->GetRootNode(), true);
    }
...
public:
  ...
  static Handle(exeFeatures_CommonFacilities) Instance()
  {
    if ( m_ref.IsNull() )
      m_ref = new exeFeatures_CommonFacilities;
...
private:
  ...
  static Handle(exeFeatures_CommonFacilities) m_ref; //!< Single instance of facilities.
