...
#include <gui_dialog_STEP_delegate.h>
...
gui_dialog_STEP::gui_dialog_STEP(const Mode mode,
                                 QWidget*   parent) : QDialog(parent), m_mode(mode)
{
  ...
  m_widgets.pOptions->setItemDelegateForColumn( 1, new gui_dialog_STEP_delegate(this) );
...
//! Destructor.
gui_dialog_STEP::~gui_dialog_STEP()
{
...
void gui_dialog_STEP::initialize()
{
  // Activate static variables of STEP translation tool
  STEPControl_Controller::Init();

  // Collect variables
  NCollection_Sequence<common_var> vars;
  //
  if ( m_mode == Mode_Read )
  ...
  for ( int v = 1; v <= vars.Length(); ++v )
  {
    ...
    pValueItem->setData( gui_dialog_STEP_delegate::WidgetRole, QVariant(var.Type) );
...
void gui_dialog_STEP::initialize_Read(NCollection_Sequence<common_var>& vars)
{
  vars.Append( common_var( var_STRING,  xstep_cascade_unit,           xstep_cascade_unit_descr           ) );
  vars.Append( common_var( var_INTEGER, read_step_product_mode,       read_step_product_mode_descr       ) );
  vars.Append( common_var( var_INTEGER, read_step_product_context,    read_step_product_context_descr    ) );
  vars.Append( common_var( var_INTEGER, read_step_shape_repr,         read_step_shape_repr_descr         ) );
  vars.Append( common_var( var_INTEGER, read_step_assembly_level,     read_step_assembly_level_descr     ) );
  vars.Append( common_var( var_INTEGER, read_step_shape_relationship, read_step_shape_relationship_descr ) );
  vars.Append( common_var( var_INTEGER, read_step_shape_aspect,       read_step_shape_aspect_descr       ) );
  vars.Append( common_var( var_INTEGER, read_precision_mode,          read_precision_mode_descr          ) );
  vars.Append( common_var( var_REAL,    read_precision_val,           read_precision_val_descr           ) );
  vars.Append( common_var( var_INTEGER, read_maxprecision_mode,       read_maxprecision_mode_descr       ) );
  vars.Append( common_var( var_REAL,    read_maxprecision_val,        read_maxprecision_val_descr        ) );
  vars.Append( common_var( var_STRING,  read_step_resource_name,      read_step_sequence_descr           ) );
  vars.Append( common_var( var_STRING,  read_step_sequence,           read_step_sequence_descr           ) );
  vars.Append( common_var( var_INTEGER, read_surfacecurve_mode,       read_surfacecurve_mode_descr       ) );
  vars.Append( common_var( var_INTEGER, read_stdsameparameter_mode,   read_stdsameparameter_mode_descr   ) );
  vars.Append( common_var( var_REAL,    read_encoderegularity_angle,  read_encoderegularity_angle_descr  ) );
  vars.Append( common_var( var_INTEGER, read_step_nonmanifold,        read_step_nonmanifold_descr        ) );
}

//! Initializes variable for writing.
//! \param vars [out] initialized variables.
void gui_dialog_STEP::initialize_Write(NCollection_Sequence<common_var>& vars)
{
  vars.Append( common_var( var_INTEGER, write_step_schema,        write_step_schema_descr       ) );
  vars.Append( common_var( var_STRING,  write_step_product_name,  write_step_product_name_descr ) );
  vars.Append( common_var( var_STRING,  write_step_unit,          write_step_unit_descr         ) );
  vars.Append( common_var( var_INTEGER, write_step_assembly,      write_step_assembly_descr     ) );
  vars.Append( common_var( var_STRING,  write_step_resource_name, write_step_sequence_descr     ) );
  vars.Append( common_var( var_STRING,  write_step_sequence,      write_step_sequence_descr     ) );
  vars.Append( common_var( var_INTEGER, write_surfacecurve_mode,  write_surfacecurve_mode_descr ) );
  vars.Append( common_var( var_INTEGER, write_precision_mode,     write_precision_mode_descr    ) );
  vars.Append( common_var( var_REAL,    write_precision_val,      write_precision_val_descr     ) );
  vars.Append( common_var( var_INTEGER, write_step_vertex_mode,   write_step_vertex_mode_descr  ) );
  vars.Append( common_var( var_INTEGER, write_step_nonmanifold,   write_step_nonmanifold_descr  ) );
}
...
void gui_dialog_STEP::saveVars()
{
  ...
  for ( int row = 0; row < m_widgets.pOptions->rowCount(); ++row )
  {
    ...
    // Get variable type
    const int var_type = pValueItem->data(gui_dialog_STEP_delegate::WidgetRole).toInt();
...
//! Writes STEP.
void gui_dialog_STEP::proceed_Write()
{
  ...
  // Save
  if ( !geom_STEP::Write( targetShape, QStr2AsciiStr(filename) ) )
  {
...
//! Reads STEP.
void gui_dialog_STEP::proceed_Read()
{
...
void gui_dialog_STEP::onVarChanged(QTableWidgetItem* pItem)
{
  Q_UNUSED(pItem);
...
//! On proceed.
void gui_dialog_STEP::onProceed()
{
  if ( m_mode == Mode_Read )
    this->proceed_Read();
  else
