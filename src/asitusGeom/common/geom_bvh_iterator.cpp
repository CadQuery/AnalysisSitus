...
geom_bvh_iterator::geom_bvh_iterator(const NCollection_Handle< BVH_Tree<double, 4> >& bvh)
: m_bvh         (bvh),
  m_iStackHead  (-1),
...
//!         false -- otherwise.
bool geom_bvh_iterator::More() const
{
  if ( m_currentNode == -1 )
    return false;
...
//! \return current item.
const BVH_Vec4i& geom_bvh_iterator::Current() const
{
...
int geom_bvh_iterator::CurrentIndex() const
{
...
bool geom_bvh_iterator::IsLeaf() const
{
...
void geom_bvh_iterator::BlockLeft()
{
...
void geom_bvh_iterator::BlockRight()
{
