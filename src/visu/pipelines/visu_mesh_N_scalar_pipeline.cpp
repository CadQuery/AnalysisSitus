...
// Own include
#include <visu_mesh_N_scalar_pipeline.h>
...
visu_mesh_N_scalar_pipeline::visu_mesh_N_scalar_pipeline()
                   ...
                   vtkSmartPointer<vtkActor>::New() )
{
  ...
  vtkSmartPointer<visu_mesh_N_scalar_filter>
    aScFilter = vtkSmartPointer<visu_mesh_N_scalar_filter>::New();
...
void visu_mesh_N_scalar_pipeline::SetInput(const Handle(visu_data_provider)& theDataProvider)
{
  Handle(visu_mesh_N_scalar_data_provider)
    aMeshPrv = Handle(visu_mesh_N_scalar_data_provider)::DownCast(theDataProvider);
  ...
  if ( aMeshPrv->MustExecute( this->GetMTime() ) )
  {
    ...
    visu_mesh_N_scalar_filter*
      aScFilter = visu_mesh_N_scalar_filter::SafeDownCast( m_filterMap.Find(Filter_NScalar) );
...
void visu_mesh_N_scalar_pipeline::addToRendererCallback(vtkRenderer*)
{
  this->Actor()->GetProperty()->SetInterpolationToGouraud();
}

//! Callback for RemoveFromRenderer base routine.
void visu_mesh_N_scalar_pipeline::removeFromRendererCallback(vtkRenderer*)
{
}

//! Callback for Update routine.
void visu_mesh_N_scalar_pipeline::updateCallback()
{
  visu_mesh_N_scalar_filter*
    aScFilter = visu_mesh_N_scalar_filter::SafeDownCast( m_filterMap.Find(Filter_NScalar) );
  aScFilter->Update();
