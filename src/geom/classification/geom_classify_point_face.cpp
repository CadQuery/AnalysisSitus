...
geom_classify_point_face::geom_classify_point_face(const TopoDS_Face& F,
...
geom_membership
  geom_classify_point_face::operator()(const gp_Pnt2d&      PonS,
...
//! \return classification result.
geom_membership
  geom_classify_point_face::operator()(const gp_Pnt&        P,
                                       const bool           checkGap,
                                       ActAPI_ProgressEntry Journal)
{
  gp_Pnt2d UV;
...
#if defined DRAW_DEBUG
  DRAW_INITGROUP(geom_classify_point_face)
  DRAW_POINT(UV, geom_classify_point_face, Draw_rouge, Draw_Plus)
#endif
