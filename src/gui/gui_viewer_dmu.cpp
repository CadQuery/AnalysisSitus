...
gui_viewer_dmu::gui_viewer_dmu(QWidget* parent) : gui_viewer(parent)
{
  // Initialize Presentation Manager along with QVTK widget
  common_facilities::Instance()->Prs.DMU = vtkSmartPointer<visu_prs_manager>::New();
  m_prs_mgr = common_facilities::Instance()->Prs.DMU;
  //
  m_prs_mgr->Initialize(this);
  m_prs_mgr->SetInteractionMode(visu_prs_manager::InteractionMode_3D);

  // Widgets and layouts
  gui_controls_dmu* pControlPane = new gui_controls_dmu(this);
...
//! Updates viewer.
void gui_viewer_dmu::Repaint()
{
  m_prs_mgr->GetQVTKWidget()->repaint();
}

//! Resets view.
void gui_viewer_dmu::onResetView()
{
