...
void journal_notifier::Init(const int theCapacity)
{
  // Clean up the cumulative collections
  m_CMap.clear();
...
int journal_notifier::Capacity() const
{
  return m_iCapacity;
}
...
//! \param theMsgKey [in] message key to set.
void journal_notifier::SetMessageKey(const TCollection_AsciiString& theMsgKey)
{
  m_msgKey = theMsgKey;
...
TCollection_AsciiString journal_notifier::MessageKey() const
{
  return m_msgKey;
}
...
void journal_notifier::SetProgressStatus(const ActAPI_ProgressStatus theStatus)
{
  m_status = theStatus;
...
ActAPI_ProgressStatus journal_notifier::ProgressStatus() const
{
  return m_status;
}

//! Sends cancellation requests to the task context.
void journal_notifier::Cancel()
{
...
int journal_notifier::SummaryProgress() const
{
  int aResult = 0;
...
void journal_notifier::DumpProgressMap(Standard_OStream& theOS) const
{
  for ( CMap::const_iterator cit = m_CMap.begin(); cit != m_CMap.end(); cit++ )
  {
...
void journal_notifier::StepProgress(const int theTaskID,
...
void journal_notifier::SendLogMessage(const TCollection_AsciiString&  theMessage,
...
//! \param thePriority  [in] message priority (normal, high).
//! \param theArguments [in] message arguments (if any).
void journal_notifier::SendLogMessage(const ActAPI_LogStream& theLogStream)
{
