...
void geom_point_cloud::RemoveCoincidentPoints(const double                 prec,
                                              geom_intersection_points_cc& pts)
{
  geom_intersection_points_cc purifiedList;
  int ip = 1;
  do
  {
    const Handle(geom_intersection_point_cc)& p = pts(ip);

    // Traverse till the end skipping all coincident points
    int jp = ip + 1;
    Handle(geom_intersection_point_cc) p_next, p_best = p;
    for ( ; ; )
    {
      ip = jp;
...
Handle(geom_point_cloud)
  geom_point_cloud::FromIntersection(const geom_intersection_points_cc& pts)
{
...
geom_point_cloud::geom_point_cloud(ActAPI_ProgressEntry progress,
...
//! \param progress [in] progress indication.
//! \param plotter  [in] imperative plotter.
geom_point_cloud::geom_point_cloud(const Handle(TColStd_HArray1OfReal)& coords,
...
//! \return true in case of success, false -- otherwise.
bool geom_point_cloud::Load(const TCollection_AsciiString& filename)
{
...
//! \return true in case of success, false -- otherwise.
bool geom_point_cloud::readPositions(std::ifstream&       FILE,
